<template>
  <div class="flex h-full flex-col">
    <div class="flex flex-wrap items-center gap-2 border-b border-gray-200 bg-white px-4 py-1.5 sm:px-6">
      ...
      <span class="text-xs text-gray-400">Drag a bar to reschedule</span>
    </div>
    ...
    <div class="min-h-0 flex-1 overflow-auto p-4">
      ...
      >
        No tasks have start and due dates yet. Set dates on a task to place it on the timeline.
      </p>
...
function ganttTasks() {
  ...
  return scheduled.value.map((task) => {
    const classes = ['gantt-status-' + task.status.replace(/[^a-z]/gi, '').toLowerCase()]
    if (task.is_critical) classes.push('gantt-critical')
    if (task.is_blocked) classes.push('gantt-blocked')
    return {
      id: task.name,
      name: task.subject,
      ...
      // only link to predecessors that are actually on the chart
...
function persistDates(taskName, start, end) {
  updateDates
    .submit({
      task: taskName,
      exp_start_date: toDateStr(start),
      exp_end_date: toDateStr(end),
    })
    .then(() => {
      toast({ title: 'Dates updated', type: 'success', timeout: 2000 })
      timeline.reload()
      emit('changed')
    })
    .catch((err) => {
...
.agile-gantt .gantt-statusdone .bar-progress {
  fill: #22c55e;
}
</style>
